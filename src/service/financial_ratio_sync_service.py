"""재무비율 동기화 서비스 — KIS financial_ratio → `stock_financial_ratios`.

대량(전종목 × 연간) 수집. 핵심 안전장치(income_statement_sync_service와 동일):
- KIS API 호출 루프를 **DB 트랜잭션 밖**에서 수행하고, **청크 단위로 독립 session_scope**를
  열어 커밋 → 장기 idle-in-transaction / QueuePool 고갈 회피.
- 증분 가드: 이미 최신 사업보고서(연간)를 커버한 종목은 skip(정상 cron 호출량 감소). 백필은 skip_current=False.
...
from src.providers.kis_financial_ratio_client import FinancialRatioRow, KisFinancialRatioClient

logger = logging.getLogger(__name__)


@dataclass(frozen=True)
class FinancialRatioSyncResult:
    """sync 결과 통계 (의미 분리)."""
...
class FinancialRatioSyncService:
    """KIS 재무비율을 `stock_financial_ratios`에 동기화."""

    def __init__(
            self,
            database: Database,
            kis_client: KisFinancialRatioClient,
            chunk_size: int = 200,
    ...
    def sync(self, skip_current: bool = True, now: date | None = None) -> FinancialRatioSyncResult:
        """active KR_STOCK 전종목 재무비율(연간) 동기화.

        Args:
            skip_current: True면 이미 최신 사업보고서(연간)를 커버한 종목 skip(정상 cron). 백필은 False.
            now: 증분 가드 기준일(테스트용). 미지정 시 호출 시점.
        """
        targets = self._load_targets()
        today = now or date.today()
        latest = self._latest_by_ticker() if skip_current else {}
        expected = _latest_expected_annual_stac_yymm(today)

        buffer: list[StockFinancialRatio] = []
        ticker_count = 0
        skipped_current = 0
        ...
        for ticker_id, code in targets:
            ...
            if skip_current and latest.get(ticker_id, "") >= expected:
                skipped_current += 1
                continue

            api_calls_attempted += 1
            try:
                rows = self._kis.fetch(code)
            except Exception as e:
                logger.warning("재무비율 조회 실패 ticker=%s: %s", code, e)
                api_calls_failed += 1
                continue
            finally:
        ...
        result = FinancialRatioSyncResult(
        ...
        logger.info(
            "재무비율 동기화 완료 tickers=%d skipped_current=%d api_attempt=%d api_fail=%d "
    ...
    def _latest_by_ticker(self) -> dict[int, str]:
        with self._database.session_scope() as session:
            return StockFinancialRatioRepository(session).latest_stac_yymm_by_ticker()

    def _commit_chunk(self, entities: list[StockFinancialRatio]) -> tuple[int, bool]:
        """청크를 독립 트랜잭션으로 커밋. (반영 행 수, 성공 여부)."""
        try:
            with self._database.session_scope() as session:
                StockFinancialRatioRepository(session).bulk_upsert(entities)
            return len(entities), True
        except Exception:
            logger.exception("재무비율 청크 커밋 실패 (rows=%d)", len(entities))
            return 0, False


def _to_entities(ticker_id: int, rows: list[FinancialRatioRow]) -> list[StockFinancialRatio]:
    return [
        StockFinancialRatio(
            ticker_id=ticker_id,
            ...
            reserve_rate=r.reserve_rate,
            revenue_growth=r.revenue_growth,
            op_growth=r.op_growth,
...
def _latest_expected_annual_stac_yymm(today: date) -> str:
    ...
    그 전이면 전전년(YYYY-2 12). income_statement_sync_service와 동일 매핑.
    """
    y = today.year
    if today >= date(y, 4, 8):
        return f"{y - 1}12"
