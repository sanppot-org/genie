...
### 후속 (실행 완료 2026-06-10)
prod 광역 재동기화(from=2023-12-01, 사용자 1회 한정 승인하에 실행) → received 35,460/upserted 4,428. KG케미칼 fiscal_year 2025 보정·연속 복원 확인. 117종목 중 **114 보정, 3 잔존**.

### 알려진 한계 — placeholder 미발급 (잔존 3)
KIS가 `(N-1)-12-31 dps=0` placeholder를 모든 제도변경 종목/연도에 주지 않음. 농심(placeholder 전무)·삼천리(2025 누락) 미보정, SV인베스트먼트(상시 3월 결산)는 미보정 정당. 강화안("과거 Dec 결산 이력 보유" 판별)은 **결산월 변경(12월→3월) 종목 오보정 + 이력조건 시점 의존성** 리스크로 codex 교차검증서 기각, **현재 placeholder 규칙 수용**(사용자 결정). DART 결산월 교차가 확실 차단책이나 효용 대비 보류 — 구조적 재발 가능, 추후 재검토.
