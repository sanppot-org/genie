"""손익계산서(매출·영업이익·순이익) 백필 스크립트.

사용:
    # 전 active KR_STOCK 초기 적재 (증분 가드 OFF로 전량)
    uv run python scripts/backfill_income_statements.py

    # 증분만 (이미 최신 분기 커버 종목 skip) — cron과 동일 동작 수동 실행
    uv run python scripts/backfill_income_statements.py --incremental

종목당 2 calls(연간/분기). 청크 단위 독립 트랜잭션 커밋이라 중단돼도 부분 보존 +
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill stock_income_statements for all active KR_STOCK.")
    parser.add_argument(
        "--incremental",
        action="store_true",
        help="이미 최신 분기를 커버한 종목 skip (기본: 전량 적재)",
    )
    ...
    service = container.income_statement_sync_service()
