"""Tests for BuybackSyncService (인메모리 sqlite + mock DART client)."""
...
def kr_stock_ticker_id(db: Database) -> int:
    """NAVER KR_STOCK 1개 적재 후 ticker_id 반환 (세션 분리 안전)."""
...
def _make_service(
    ...
    return BuybackSyncService(database=db, client=client)


def test_sync_upserts_acquisition_and_disposal(db: Database, kr_stock_ticker_id: int) -> None:
    ...
    result = _make_service(db, responses).sync(date(2024, 1, 1), date(2024, 12, 31))
    ...
    with db.session_scope() as session:
        rows = StockBuybackEventRepository(session).find_by_ticker(kr_stock_ticker_id)
        assert {r.event_type for r in rows} == {"ACQUISITION", "DISPOSAL"}
        assert {r.rcept_no for r in rows} == {"20240930000001", "20240326000771"}


def test_sync_counts_failure(db: Database, kr_stock_ticker_id: int) -> None:
    """클라이언트 예외는 skipped_failure로 카운트."""
    result = _make_service(db, {"035420": RuntimeError("boom")}).sync(
...
def test_sync_is_idempotent(db: Database, kr_stock_ticker_id: int) -> None:
    ...
    _make_service(db, {"035420": [first]}).sync(date(2024, 1, 1), date(2024, 12, 31))
    ...
    _make_service(db, {"035420": [second]}).sync(date(2024, 1, 1), date(2024, 12, 31))
