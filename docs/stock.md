...
[ ] 스크리닝
  ...
  [ ] 필터링
    ...
    [x] 종목 검색
    [x] 분기, 연속, 총점
  [x] 조건 필터 스크리너 Phase 1 — 규칙 엔진 + `POST /screening/filter`: 영업이익/EPS 등 연간 시계열 조건(count/cagr/streak/avg) AND 필터, total_score 지표 공유, 데이터부족 종목 제외. (스코어러와 분리된 별도 엔드포인트, DB 변경 없음)
    [x] Phase 2: 웹 규칙빌더 UI — `/screening/filter` 페이지(범용 op-aware 조건 행 빌더, 동적 컬럼 결과 테이블, 스칼라/종합점수 정렬·offset 페이지네이션). 헤더 "필터" 링크. 지표 카탈로그는 프론트 하드코딩(백엔드 METRIC_REGISTRY와 수동 동기화).
